...
Route::Route():
	...
	index("index.html"), static_dir(),
	log(Logger::instance(), "Route"),
	ev(NULL)
{
}
...
{	
	// std::cout << "root_directory: " << root_directory << std::endl;
...
std::string Route::build_absolute_path(Request req)
{
	...
	StrContainer	req_path(req.getUrl().getPath());
	if (root.ends_with("/"))
...
void Route::printRoute()
{
	...
	this->log << LOG_INFO << "                "<< "Route type: " << this->getType();
	this->log << LOG_INFO << "                dir_listing: " << this->dir_listing;
	this->log << LOG_INFO << "                root_directory: " << this->root_directory;
	this->log << LOG_INFO << "                redirect_url: " << this->redirect_url;
	this->log << LOG_INFO << "                index: " << this->index;
	this->log << LOG_INFO << "                static_dir: " << this->static_dir;
	for (std::vector<std::string>::iterator it = this->file_extensions.begin(); it != this->file_extensions.end(); it++)
		this->log << LOG_INFO << "                file_extension: " << *it;
	for (std::vector<Method>::iterator it = this->allowed_methods.begin(); it != this->allowed_methods.end(); it++)
		this->log << LOG_INFO << "                allowed_method: " << *it;
	this->log << LOG_INFO << "                *";
}
...
IHandler *Route::route(IData &request, StringData &error)
{
	...
	else if (this->type == CGI_)
	{
		...
		StrContainer cgi_path = this->cgi.pathToScript(
			this->root_directory,
			this->index,
	...
	{
		//this->log << LOG_INFO << "no route found";
