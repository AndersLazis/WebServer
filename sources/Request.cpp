...
void	Request::setVersion(StrContainer version)
{
...
StrContainer Request::getVersion() const
{
...
StringData Request::save_chunk(std::string output_file)
{
	...
	if (this->content_length > 0)
	{
		...
		//this->log << LOG_INFO << output_file << ": saved " << output.tellp() << " from " << this->content_length;
		if (this->content_length <= output.tellp())
		{
			output.close();
			this->chunked_state = CH_COMPLETE;
			this->log << LOG_INFO << output_file << ": completed";
		...
		while (processing)
		{
			switch (this->chunked_state)
			{
				...
				case CH_DATA:
				{
					...
					//this->log << LOG_INFO << output_file << ": saved " << output.tellp() << " from " << this->content_length << ", remaining chunk size: " << this->remaining_chunk_size;
				...
				case CH_ERROR:
					//this->chunked_state = CH_START;
					this->log << LOG_INFO << output_file << ": error";
					return StringData("500");
					break;
				default:
					break;
