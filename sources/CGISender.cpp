...
CGISender::CGISender()
         : fd(-1),
				   ready(false), _finished(false),
					 pos(0),
					 log(Logger::instance(), "CGISender")
{
}

CGISender::CGISender(int fd)
         : fd(fd),
				   ready(false), _finished(false),
					 pos(0),
					 log(Logger::instance(), "CGISender")
...
{
	//this->log << LOG_INFO << "TMP FILE: " << this->tmp_file;
	if (!this->tmp_file.empty())
	{
		...
		if (sent > 0)
			this->pos += sent;
		//this->log << LOG_INFO << this << " SENT TO CGI: " << sent << ", pos: " << this->pos << "/" << file_size << ", file: " << this->tmp_file;
